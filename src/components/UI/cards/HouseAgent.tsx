...
const HouseAgent: React.FC<PropertyCardProps> = ({
  ...
  return (
    <div className=" w-full md:w-[23rem]  h-fit flex rounded-xl flex-col items-center justify-center relative" >
      ...
      <div className="w-full pt-2 bg-white shadow-2xl  rounded-b-xl">
        <div className="flex flex-col px-5 gap-3">
          ...
          <div className="flex flex-col pb-5 border-b border-gray-200 gap-1">
            <p className="text-base text-primary_gray font-jost ">{fullHouseData.title}</p>
            <p className="text-xs font-normal text-primary_gray ">{fullHouseData.description?.length > 50 ? `${fullHouseData.description.substring(0, 100)}` : fullHouseData.description}</p>
          </div>
        </div>
